...
def _validate_schema(schema: str) -> None:
    if not _SCHEMA_RE.match(schema):
        raise CreditsError("invalid_database_schema", "Database schema name is invalid.", 500)
...
class Database:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            raise CreditsError("database_unavailable", "PostgreSQL is unavailable or schema bootstrap failed.", 503) from exc
    ...
    async def pool(self) -> asyncpg.Pool:
        ...
        if self._pool is None:
            raise CreditsError("database_unavailable", "PostgreSQL is unavailable.", 503)
        return self._pool
