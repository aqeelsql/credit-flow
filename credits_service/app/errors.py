...
def register_error_handlers(app: FastAPI) -> None:
    @app.exception_handler(CreditsError)
    async def credits_error_handler(request: Request, exc: CreditsError):
    ...
    async def unhandled_error_handler(request: Request, exc: Exception):
        return JSONResponse(status_code=500, content=error_payload("internal_error", "An unexpected credits service error occurred.", request))
