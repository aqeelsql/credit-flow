...
class UsageEventBus:
    ...
    async def connect(self) -> None:
        ...
        await dlq.bind(self._exchange, routing_key="usage.failed.dlq")
        await self._channel.declare_queue(self.settings.retry_queue, durable=True, arguments={"x-message-ttl": self.settings.retry_delay_ms, "x-dead-letter-exchange": self.settings.rabbitmq_exchange, "x-dead-letter-routing-key": "ai.generation_completed"})
        self._queue = await self._channel.declare_queue(self.settings.rabbitmq_queue, durable=True, arguments={"x-dead-letter-exchange": self.settings.rabbitmq_exchange, "x-dead-letter-routing-key": "usage.failed.dlq"})
        await self._queue.bind(self._exchange, routing_key="ai.generation_completed")
    ...
    async def publish_retry(self, routing_key: str, payload: dict[str, Any], retry_count: int) -> None:
        await self.connect()
        if self._channel is None:
            raise RuntimeError("RabbitMQ channel is not connected")
        message = Message(json.dumps(payload, default=str).encode("utf-8"), content_type="application/json", delivery_mode=DeliveryMode.PERSISTENT, headers={"x-retry-count": retry_count, "x-original-routing-key": routing_key})
    ...
    async def _consume(self, message: aio_pika.IncomingMessage) -> None:
        async with message.process(ignore_processed=True, requeue=False):
            try:
                ...
                logging.warning("Dropped malformed %s message", message.routing_key)
                return
    ...
    async def close(self) -> None:
        ...
        self._queue = None; self._exchange = None; self._channel = None; self._connection = None
