...
def _validate_schema(schema: str) -> None:
    if not _SCHEMA_RE.match(schema):
        raise GenerationError("invalid_database_schema", "Database schema name is invalid.", 500)
...
class Database:
    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            self._pool = await asyncpg.create_pool(
                ...
                max_size=10,
                command_timeout=30,
                server_settings={"search_path": f"{self.settings.database_schema},public"},
            )
        except Exception as exc:
            raise GenerationError("database_unavailable", "PostgreSQL is unavailable or schema bootstrap failed.", 503) from exc
    ...
    async def pool(self) -> asyncpg.Pool:
        ...
        if self._pool is None:
            raise GenerationError("database_unavailable", "PostgreSQL is unavailable.", 503)
        return self._pool
