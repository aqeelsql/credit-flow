...
export default function TeamPage() {
  return (
    <RouteGuard allowedRoles={["Owner", "TenantAdmin"]}>
      <TeamManagement />
    </RouteGuard>
  );
}
...
function TeamManagement() {
  ...
  const [memberUsage, setMemberUsage] = useState<Record<string, MemberUsageRow>>({});
  ...
  const loadMembers = useCallback(async () => {
    ...
    try {
      const [rows, usageRows] = await Promise.all([
        ...
        api<MemberUsageRow[]>("/api/credits/member-usage", accessToken).catch(() => []),
      ]);

      setMemberUsage(Object.fromEntries(usageRows.map((row) => [row.user_id, row])));
      setMembers(
        rows.map((row) => ({
          id: row.id,
          user_id: row.user_id,
          ...
          joinedViaInvite: Boolean(row.joined_via_invite),
        }))
      );
    } catch (err) {
    ...
    <section className="page">
      <div className="page-header">
        <div>
          ...
          <p className="page-subtitle">Invite members, manage roles, and monitor shared credit usage across the workspace.</p>
        </div>
        <div className="button-row">
          <span className="status-badge neutral">{activeTeamMembers.length} team member{activeTeamMembers.length === 1 ? "" : "s"}</span>
          <span className="status-badge live">{inviteJoinedMembers.length} joined by invite</span>
        </div>
      </div>
      ...
      <div className="table-panel">
        <div className="table-header">
          <h2>Members</h2>
          <span className="status-badge neutral">{members.length} total including owner</span>
        </div>
        {members.length ? (
          <table className="data-table">
            ...
            <tbody>
              {members.map((member) => {
                const usage = member.user_id ? memberUsage[member.user_id] : undefined;
                return (
                  <tr key={member.id}>
                    <td>{member.name}</td>
                    <td>{member.email}</td>
                    <td>
                      <select className="select" value={member.role} disabled={member.role === "Owner"} onChange={(event) => void updateRole(member, event.target.value as TeamMember["role"])}>
                    ...
                    <td>
                      <span className="mono">{(usage?.credits_used ?? 0).toLocaleString()}</span>
                      <div className="muted-text small-text">{usage?.generation_count ? `${usage.generation_count.toLocaleString()} post${usage.generation_count === 1 ? "" : "s"}` : "No AI usage yet"}</div>
                    </td>
                    <td>{member.joinedViaInvite ? <span className="status-badge live">Yes</span> : <span className="status-badge neutral">No</span>}</td>
                    <td><span className={`status-badge ${member.status === "Active" ? "success" : "warning"}`}>{member.status}</span></td>
                    <td><button className="icon-button danger" type="button" aria-label={`Remove ${member.name}`} disabled={member.role === "Owner"} onClick={() => setPendingRemoval(member)}><Trash2 size={16} aria-hidden="true" /></button></td>
                  </tr>
                );
              })}
            </tbody>
          ...
          <div className="empty-state">No team members returned yet.</div>
        )}
      </div>
