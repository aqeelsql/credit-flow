...
class EventPublisher:
    ...
    async def publish(self, routing_key: str, payload: dict) -> None:
        try:
            ...
            raise SchedulerError("events_unavailable", "RabbitMQ is unavailable.", 503) from exc
        if self._exchange is None:
            raise SchedulerError("events_unavailable", "RabbitMQ is unavailable.", 503)
        message = Message(json.dumps(payload, default=str).encode("utf-8"), content_type="application/json", delivery_mode=DeliveryMode.PERSISTENT)
