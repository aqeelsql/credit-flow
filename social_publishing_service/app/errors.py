...
def error_payload(code: str, message: str, request: Request | None = None, details=None) -> dict:
    return {"error": {"code": code, "message": message, "request_id": getattr(request.state, "request_id", None) if request else None, "details": details}}


def register_error_handlers(app: FastAPI) -> None:
    @app.exception_handler(SocialPublishingError)
    async def social_error_handler(request: Request, exc: SocialPublishingError):
    ...
    async def unhandled_error_handler(request: Request, exc: Exception):
        return JSONResponse(status_code=500, content=error_payload("internal_error", "An unexpected social publishing service error occurred.", request))
