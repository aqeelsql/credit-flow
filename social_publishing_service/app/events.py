...
class SocialEventBus:
    ...
    async def connect(self) -> None:
        ...
        await dlq.bind(self._exchange, routing_key="post.failed.dlq")
        await self._channel.declare_queue(
            ...
            arguments={
                ...
                "x-dead-letter-routing-key": "content.scheduled",
            },
        )
        self._queue = await self._channel.declare_queue(
            self.settings.rabbitmq_queue,
            durable=True,
            arguments={"x-dead-letter-exchange": self.settings.rabbitmq_exchange, "x-dead-letter-routing-key": "post.failed.dlq"},
        )
        await self._queue.bind(self._exchange, routing_key="content.scheduled")
    ...
    async def _consume(self, message: aio_pika.IncomingMessage) -> None:
        async with message.process(ignore_processed=True, requeue=False):
            try:
                ...
                logging.warning("Dropped malformed content.scheduled message")
                return
